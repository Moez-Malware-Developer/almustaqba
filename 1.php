...
function calculateDues($conn, $technician) {
    // هذه الوظيفة تتطلب اتصالاً مفتوحًا
    ...
    $sql_repairs = "SELECT SUM(agreed_price - parts_cost) AS total_profit FROM repairs WHERE assigned_to = ? AND status = 'مكتملة'";
    ...
    switch ($action) {
        ...
        case 'delete':
            ...
            if (!empty($technicianId)) {
                ...
                try {
                    $sql_payments = "DELETE FROM technician_payments WHERE technician_id = ?";
                    $stmt_payments = $conn->prepare($sql_payments);
                    $stmt_payments->bind_param("i", $technicianId);
                    $stmt_payments->execute();
                    $stmt_payments->close();
...
<body class="flex flex-col lg:flex-row min-h-screen">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // --- Form & Modal Logic ---
            agreementType.addEventListener('change', () => {
                const type = agreementType.value;
                percentageField.classList.remove('active');
                salaryField.classList.remove('active');
                
                // Clear values when switching types to avoid incorrect data submission
            ...
            addTechnicianBtn.addEventListener('click', () => {
                ...
                agreementType.dispatchEvent(new Event('change'));
            ...
            const filterTechnicians = () => {
                ...
                rows.forEach(row => {
                    ...
                    const matchesSpecialty = specialtyTerm === '' || specialty.includes(specialtyTerm);
                ...
                const noResultsRow = techniciansList.querySelector('.no-results-row');
                if (!foundMatch && rows.length > 0) {
                     if (!noResultsRow) {
                        const newRow = document.createElement('tr');
                        newRow.className = 'no-results-row';
                        newRow.innerHTML = `<td colspan="8" class="text-center py-4 text-gray-500">لا يوجد فنيين مطابقين للبحث</td>`;
                        techniciansList.appendChild(newRow);
                    } else {
            ...
            // --- Action Functions (now handled by forms) ---
            const attachActionListeners = () => {
                // Edit
                document.querySelectorAll('.edit-btn').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        const { id, technicianName, technicianPhone, nationality, specialty, agreementType, value } = e.currentTarget.dataset;
                        formTitle.textContent = 'تعديل بيانات الفني';
                        technicianIdInput.value = id;
                        actionInput.value = 'update';
                        technicianNameInput.value = technicianName;
                        technicianPhoneInput.value = technicianPhone;
                        nationalityInput.value = nationality;
                        specialtyInput.value = specialty;
                        
                        document.getElementById('agreementType').value = agreementType;
                        
                        if (agreementType === 'percentage') {
                        ...
                        document.getElementById('agreementType').dispatchEvent(new Event('change'));
                        technicianFormModal.classList.remove('hidden');
                        window.scrollTo({ top: 0, behavior: 'smooth' });
                    });
                });

                // View Payments (still uses AJAX as it's a data retrieval action)
                document.querySelectorAll('.view-payments-btn').forEach(btn => {
                    btn.addEventListener('click', async (e) => {
                        const id = e.currentTarget.dataset.id;
                        const technicianName = e.currentTarget.closest('tr').cells[0].textContent;
                        ...
                        try {
                            const response = await fetch('technicians.php', {
                                method: 'POST',
                                body: formData
                            });
                            ...
                            if (data.status === 'success' && data.payments.length > 0) {
                                ...
                                data.payments.forEach(payment => {
                                    ...
                                    row.innerHTML = `
                                        <td class="p-2 border border-gray-300">${new Date(payment.payment_date).toLocaleDateString('ar-SA')}</td>
                                        <td class="p-2 border border-gray-300 font-semibold">${parseFloat(payment.amount).toLocaleString()} ${currency}</td>
                                    `;
                            ...
                            showToast('حدث خطأ في جلب سجل المدفوعات.', 'error');
                        }
                    });
                });
            };
